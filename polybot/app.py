...
try:
    ...
    POLYBOT_RESULTS_URL = os.environ.get('POLYBOT_RESULTS_URL')
...
class Bot:
    ...
    def handle_photo_message(self, msg):
        ...
        try:
            ...
            self.send_text(chat_id, "Image sent for processing. Please wait...")
...
def status():
    return jsonify({"status": "ok", "telegram_bot_info": bot.telegram_bot_client.get_me(), "timestamp": time.time()})

# Define the message handler for Telegram
...
def main():
    ...
    # Start the Flask server on port 8443
    app.run(host='0.0.0.0', port=8443)

    # Start the Telegram bot polling in a separate thread (optional, Flask might block)
    # import threading
    # threading.Thread(target=bot.telegram_bot_client.infinity_polling, daemon=True).start()
    logger.info("Flask server started on port 8443.")
